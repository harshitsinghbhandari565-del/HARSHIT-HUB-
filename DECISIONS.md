...
- **Date:** 2026-08-05
### D-047 — JSON-LD injection via escaped serialization (documented set:html exception) + import.meta.env.SITE

- **Context:** the Phase 7 compliance audit found TAD §17.2 (canonical/OG/Twitter metadata) and §17.3 (JSON-LD structured data) unimplemented. Implementing them required injecting serialized JSON into `<script type="application/ld+json">` bodies. Astro treats `<script>` bodies as raw text — template interpolation does not run inside them (verified empirically) — so the data cannot be emitted as a template expression; the framework's canonical pattern is `set:html`, which TAD §19.2 bans categorically as an XSS control.
- **Reasoning:** the two parts of the decision:
  1. **Injection:** `serializeJsonLd()` (shared/lib/jsonLd) wraps `JSON.stringify(...).replace(/</g, '\u003c')` — the single place where the injection happens, used via `set:html` in SeoHead. This is a documented exception to the §19.2 ban, not a relaxation of it: (a) the payload is build-time data derived from Zod-validated content — user input never reaches it; (b) with every `<` escaped, the output can contain no markup and can never close the script element early (the sole mechanism by which a JSON payload becomes executable HTML); (c) `\u003c` is character-identical to JSON parsers. The ban remains enforced for everything else (no other set:html exists in the codebase).
  2. **Site URL source:** canonical/og:url are resolved against `import.meta.env.SITE` rather than `Astro.site` — the source Astro itself now recommends (Astro.site is deprecated for this use), and it resolves under the Container API where Astro.site is unavailable, keeping every BaseLayout-rendering suite green without test-only branches in production code.
- **Alternatives:** (a) JSON-LD via a client-side script — rejected (crawlers do not execute JS; defeats the purpose); (b) a custom Vite plugin transforming placeholder comments into JSON bodies — rejected (opaque build machinery for a solved problem); (c) keep Astro.site and make the Container suites skip BaseLayout — rejected (loses the highest-value structural tests); (d) a container config-injection API — does not exist in Astro 6.
- **Consequences:** covered by 10 unit tests (serializer safety against hostile payloads, builder shapes) and 11 build-output integration tests (metadata correctness, JSON-LD validity, drift against visible content). og:image/Twitter large-image emit only once a brand image exists (IA-2); until then the card type is `summary` — documented in RELEASE_CHECKLIST.
- **Date:** 2026-08-05
