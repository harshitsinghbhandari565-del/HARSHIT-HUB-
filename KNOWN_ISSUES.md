...
_All quality gates green at Phase 6 completion (typecheck 0/0/0, ESLint 0, Stylelint 0, tests 190/190, build clean — 13 routes + search index; eager budgets within TAD §14.1 on every route; contact hydration nuance tracked as TD-14)._
...
| TD-14 | Contact page effective JS with ContactForm hydration ≈16.8 KB vs the 15 KB TAD §14.1 budget for /contact — the overage is Preact itself (required by the designed form island); eager load is 6.34 KB. Recommend a minor budget revision for /contact at the Phase H budget review | Phase 6 | Phase H budget review; no code change needed unless the budget is enforced at 15 |
